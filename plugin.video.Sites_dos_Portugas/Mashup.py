...
##        log = xbmc.executebuiltin(builtin % ('A Procurar Últimos Episódios.', 'Por favor aguarde...',artfolder + 'SDPI.png'))
        ...
        try:
                ...
                itemsTPT = re.findall('<h2 class="title">(.*?)<div class="readmore">', html_source, re.DOTALL)
                if not itemsTPT: itemsTPT = re.findall('<div class="post-wrap">(.*?)<div class="readmore-wrap">', html_source, re.DOTALL)
...
def ultimos_ep_TPT(item):
        ...
        if item != '':
                try:
                        ...
                        urliii = urletitulo[0][0]
                        try:
                                html_source = abrir_url(urliii)
                        except: html_source = ''
...
##                        if 'calendar_title' in items[0]: n_items = n_items - 1
                        ...
                        try:
                                ...
                                try:
                                        ...
                                        addDir_episode1_true(nome_antes+' [COLOR grey]S'+temporadat+' x E'+episodiot+' - [/COLOR][COLOR blue]'+epi_nome+'[/COLOR]',f_id+'//[COLOR grey]S'+temporadat+' x E'+episodiot+' - [/COLOR][COLOR blue]'+epi_nome+'[/COLOR]',7000,th,str(sin),fanart,episodiot,air,temporada+'x'+episodiot+' '+namet,urltrailer,mvoutv,0)
                                        f_id = ''
                                except: pass
                        except:                                     
                                ...
                                mvoutv = temporada+'|'+episodio+'|'+namet+'|'+tvdbid+'|'+imdbcode+'|'+anne
                                addDir_episode1_true(nome_antes+' [COLOR grey]S'+temporadat+' x E'+episodiot+' - [/COLOR][COLOR blue]'+nomeepisodio+'[/COLOR]',f_id+'//[COLOR grey]S'+temporadat+' x E'+episodiot+' - [/COLOR][COLOR blue]'+nomeepisodio+'[/COLOR]',7000,fanart,'',fanart,episodiot,'',temporada+'x'+episodiot+' '+namet,urltrailer,mvoutv,0)
                                f_id = ''
...
##                #for parte1,parte2 in linkseries:
                linkseries = re.findall('border-top-right-radius:2px;">(.+?)EPISODIO LEGENDADO</div>(.+?)DOWNLOAD POR',link2,re.DOTALL)
                if not linkseries: linkseries = re.findall('<span class="su-spoiler-icon"></span>(.+?)EPISODIO LEGENDADO</div>(.+?)DOWNLOAD',link2,re.DOTALL)
                #return

...
##        return
        ...
        #return
        if 'Movies' not in mvoutv and ('Season' in nometitulo or 'Temporada' in nometitulo or 'Mini-Série' in nometitulo or 'Mini-serie' in nometitulo or 'Minisérie' in nometitulo or 'Miniserie' in nometitulo):
                num = 0
                percent = 0
                message = ''
...
##                progress.update(percent, 'A Procurar em '+site, message, "")
        ...
        if 'Movies' in mvoutv:# and 'Season' not in name and 'Temporada' not in name and 'Mini-Série' not in name and 'Mini-Serie' not in name and 'Minisérie' not in name and 'Miniserie' not in name:
	...
	if link2:
                if 'Movies' in mvoutv:# and 'Season' not in nometitulo and 'Temporada' not in nometitulo and 'Mini-Série' not in nometitulo and 'Mini-serie' not in nometitulo and 'Minisérie' not in nometitulo and 'Miniserie' not in nometitulo and imdbcode == '':
                ...
                if not newmatch:
                        if newmatch1 != [] and 'Movies' not in mvoutv and ('Season' in nometitulo or 'Temporada' in nometitulo or 'Mini-Série' in nometitulo or 'Mini-serie' in nometitulo or 'Minisérie' in nometitulo or 'Miniserie' in nometitulo):
                                ...
                                num = len(lin) + 0.0
                                lin = re.findall('EPISODIO(.*?)DOWNLOAD',link2,re.DOTALL)
                                #if not lin: lin = re.findall('.+?EPISODIO LEGENDADO',link2,re.DOTALL)
                                #addDir1(str(l),'url',1001,iconimage,False,'')
                                num = num + len(lin) + 0.0# - 1.0
                                #addLink(str(len(lin)),'','','')
                                #return
                                linkseries1 = re.findall('border-top-right-radius:2px;">(.+?)EPISODIO LEGENDADO</div>(.+?)DOWNLOAD POR',link2,re.DOTALL)
                                for parte1,parte2 in linkseries1:                                        
                                        #addLink('1','','','')
                                        try:
                                                ...
                                                #addLink(str(episodiot),'','','')
                                                if episodiot not in episodiostotais:
                                                        episimnao = 'nao'
                                                        episodiostotais.append(episodiot)
                                                else: episimnao = 'sim'
                                        except: pass
        ...
        ##                                                        conta_os_items = conta_os_items + 1
        ##                                                        fonte_id = TPT_resolve_not_videomega_filmes(url,conta_id_video,conta_os_items,nomeescolha,iconimage,fanart,mvoutv)
        ...
        ##                                                        else: f_id = f_id + '|' + fonte_id
                                ...
                                linkseries2 = re.findall('<p></div></div>\n<h2></h2>\n.+?<span class="su-spoiler-icon"></span>(.+?)</div><div class="su-spoiler-content su-clearfix"(.+?)BAIXAR',link2,re.DOTALL)
...
##                                if not linkseriesssss: linkseriesssss = re.findall('<h2>(.+?)</h2>(.+?)BAIXAR',newmatch1[0],re.DOTALL)
                               # if 'VER ONLINE LIEGENDADO' in linkseries2:
                                for parte1,parte2 in linkseries2:
                                        #episodio = re.compile('(\d+)').findall(parte1)
                                        #addLink('2 '+'-'+str(len(linkseries2)),'','','')
                                        try:
                                                episodio = re.compile('(\d+)').findall(parte1)
                                                if episodio:
                                                        episodiot = episodio[0]
        ...
        ##                                                        else: f_id = f_id + '|' + fonte_id
                                                ...
                                                try:
                                                        ...
                                                        if 'Movies' not in mvoutv and ('Season' in nometitulo or 'Temporada' in nometitulo or 'Mini-Série' in nometitulo or 'Mini-serie' in nometitulo or 'Minisérie' in nometitulo or 'Miniserie' in nometitulo):
                                                                ...
                                                                if progress.iscanceled():
                                                                        break
                                                        mvoutv = str(temporada)+'|'+str(episodio)+'|'+str(namet)+'|'+str(tvdbid)+'|'+str(imdbcode)+'|'+str(anne)
                                                        addDir_episode1_true('[COLOR grey]S'+temporadat+' x E'+episodiot+' - [/COLOR][COLOR blue]'+parte1+'[/COLOR]',f_id+'//[COLOR grey]S'+temporadat+' x E'+episodiot+' - [/COLOR][COLOR blue]'+parte1+'[/COLOR]',7000,fanart,'',fanart,episodiot,'',temporada+'x'+episodiot+' '+namet,urltrailer,mvoutv,0)
                                ...
                                linkseries3 = re.findall('<h2></h2>.+?<h2>(.+?)EPIS.+?</h2>\n(.+?)DOWNLOAD POR',link2,re.DOTALL)
                                
                                for parte1,parte2 in linkseries3:                                        
                                        #addLink('3','','','')
                                        try:
                                                episodio = re.compile('(\d+)').findall(parte1)
                                                if episodio:
        ...
        ##                                                        else: f_id = f_id + '|' + fonte_id
                                ...
                                linkseries4 = re.findall('<h2>(.+?)</h2>(.+?)BAIXAR',link2,re.DOTALL)
                                #if not linkseries4: linkseries4 = re.findall('<h2>(.+?)</h2>(.+?)DOWNLOAD POR',link2,re.DOTALL)
                                for parte1,parte2 in linkseries4:                                        
                                        #addLink('4','','','')
                                        try:
                                                episodio = re.compile('(\d+)').findall(parte1)
                                                if episodio:
        ...
        ##                                                        else: f_id = f_id + '|' + fonte_id
                                        ...
                                        i = i + 1
                                episimnao = 'sim'
                                linkseries5 = re.findall('</span>CLIQUE AQUI PARA VER O (.+?)</div>(.+?)</div></div><',link2,re.DOTALL)
                                #linkseriesssss = re.findall('<span class="su-spoiler-icon"></span>(.+?)</div>(.+?)BAIXAR',newmatch1[0],re.DOTALL)
                                #linkseriesssss = re.findall('<h2>(.+?)</h2>\n(.+?)DOWNLOAD POR',newmatch1[0],re.DOTALL)
                                #linkseriesssss = re.findall('<h2>(.+?)</h2>(.+?)BAIXAR',newmatch1[0],re.DOTALL)
                                for parte1,parte2 in linkseries5:                                        
                                        #addLink('5','','','')
                                        try:
                                                episodio = re.compile('(\d+)').findall(parte1)
                                                if episodio:
        ...
        ##                                                        else: f_id = f_id + '|' + fonte_id
                                        ...
                                        f_id = ''
                                        i = i + 1
                                #</span>CLIQUE AQUI PARA VER O (.+?)</div>(.+?)</div></div><',link2,re.DOTALL)
                                #linkseriesssss = re.findall('<span class="su-spoiler-icon"></span>(.+?)</div>(.+?)BAIXAR',newmatch1[0],re.DOTALL)
                                #linkseriesssss = re.findall('<h2>(.+?)</h2>\n(.+?)DOWNLOAD POR',newmatch1[0],re.DOTALL)
                                #linkseriesssss = re.findall('<h2>(.+?)</h2>(.+?)BAIXAR',newmatch1[0],re.DOTALL)
                ...
                if newmatch:
                        #addLink(str(len(newmatch)),'','','')
                        if 'Movies' in mvoutv:# and 'Season' not in nometitulo and 'Temporada' not in nometitulo and 'Mini-Série' not in nometitulo and 'Mini-serie' not in nometitulo and 'Minisérie' not in nometitulo and 'Miniserie' not in nometitulo:
                                try: newtch = re.findall('<h2></h2>.+?DOWNLOAD POR',link2,re.DOTALL)
                                except: pass
                                if not newtch:
                                        try: newtch = re.findall('</span>CLIQUE AQUI PARA VER O .+?</div>(.+?)</div></div><',link2,re.DOTALL)
                                        except: pass
                                if not newtch:
                                        try: newtch = re.findall('<span id="more.+?DOWNLOAD POR',link2,re.DOTALL)
                                        except: pass
                                #addLink(str(len(newtch)),'','','')
                                nfilme = 0
                                for new_match in newtch:
                                        if len(newtch)>1:
                                                nfilme = nfilme + 1
                                                addDir('[COLOR blue]Filme '+str(nfilme)+'[/COLOR]','url',9999,iconimage,'',fanart)
                                                conta_id_video = 0
                                                conta_os_items = 0
                                        match = re.compile('src="(.+?)"').findall(new_match)
                                        ...
                                        for url in match:
                                                ...
                                                if "videomega" in url or "vidto.me" in url or "video.pw" in url or "thevideo.me" in url or "dropvideo" in url or "vodlocker" in url or "played.to" in url or "cloudzilla" in url or "vidzen" in url or "vidzi.tv" in url or "divxstage" in url or "streamin.to" in url or "putlocker" in url or "nowvideo" in url or "primeshare" in url or "videoslasher" in url or "sockshare" in url or "firedrive" in url or "movshare" in url or "video.tt" in url or "videowood" in url:
                                                        conta_id_video = conta_id_video + 1
                                                        conta_os_items = conta_os_items + 1
                                                        TPT_resolve_not_videomega_filmes(url,conta_id_video,conta_os_items,nomeescolha,iconimage,fanart,mvoutv)
...
##        return
	...
	if link2:
                ...
                if newmatch:
                        if 'Movies' in mvoutv:# and 'Season' not in nometitulo and 'Temporada' not in nometitulo and 'Mini-Série' not in nometitulo and 'Mini-serie' not in nometitulo and 'Minisérie' not in nometitulo and 'Miniserie' not in nometitulo:
                                match = re.compile('src="(.+?)"').findall(newmatch[0])
                                for url in match:
                                        if "videomega" in url or "vidto.me" in url or "video.pw" in url or "thevideo.me" in url or "dropvideo" in url or "vodlocker" in url or "played.to" in url or "cloudzilla" in url or "vidzen" in url or "vidzi.tv" in url or "divxstage" in url or "streamin.to" in url or "putlocker" in url or "nowvideo" in url or "primeshare" in url or "videoslasher" in url or "sockshare" in url or "firedrive" in url or "movshare" in url or "video.tt" in url or "videowood" in url:
                                                conta_id_video = conta_id_video + 1
                                                conta_os_items = conta_os_items + 1
        ...
        for stream in streams:
                ...
                a = str(i)
                if i < 10: a = '0'+a
                TPT = threading.Thread(name='TPT'+str(i), target=TPT_resolve_not_videomega_filmes , args=(url,conta_id_video,conta_os_items,str(nomeescolha),str(iconimage),str(fanart),'Movies',))
                threads.append(TPT)

        [i.start() for i in threads]
        [i.join() for i in threads]

def TPT_resolve_not_videomega_filmes(url,conta_id_video,conta_os_items,nomeescolha,iconimage,fanart,mvoutv):
        conta_id_video = int(conta_id_video)
        conta_os_items = int(conta_os_items)
        #addLink(mvoutv,'','','')
        
        if 'Movies' in mvoutv:#'Season' not in nomeescolha and 'Temporada' not in nomeescolha and 'Mini-Série' not in nomeescolha and 'Mini-serie' not in nomeescolha and 'Minisérie' not in nomeescolha and 'Miniserie' not in nomeescolha:
                url = url + '///' + nomeescolha
		...
		try:
                        ...
                        if 'Movies' in mvoutv:#'Season' not in nomeescolha and 'Temporada' not in nomeescolha and 'Mini-Série' not in nomeescolha and 'Mini-serie' not in nomeescolha and 'Minisérie' not in nomeescolha and 'Miniserie' not in nomeescolha:
                                addDir('[B]- Fonte ' + str(conta_id_video) + ' : [COLOR yellow]'+fonte_id.replace(urltrailer,'')+'[/COLOR][/B]',urltrailer,30,iconimage,'',fanart)
		except: pass
        if "vidto.me" in url:
		try:
                        match = re.compile('http://vidto.me/embed-(.+?).html').findall(url)
	...
	if "streamin.to" in url:
    	...
    	if "video.pw" in url:
                try:
                        print url
                        fonte_id = '(Video.pw)'+url
                        if 'Movies' in mvoutv:#'Season' not in nomeescolha and 'Temporada' not in nomeescolha and 'Mini-Série' not in nomeescolha and 'Mini-Serie' not in nomeescolha:
                                addDir('[B]- Fonte ' + str(conta_id_video) + ' : [COLOR yellow]'+fonte_id.replace(url,'')+'[/COLOR][/B]',url,30,iconimage,'',fanart)
    		except:pass
    	if "thevideo.me" in url:
                try:
...
def TPT_Ultimos(name,url,iconimage,fanart,item):
	...
	if link2:
                ...
                if linkseries:
                        ...
                        #addLink(parte1+'--'+str(l)+name+str(len(linkseries)),'','','')
                        try:

...
import urllib,urllib2,re,xbmcplugin,xbmcgui,sys,xbmc,xbmcaddon,xbmcvfs,socket,threading,HTMLParser,json,requests
...
##        else: qualurl = ''
	...
	if nomesite != 'TFC':
                ...
                for it in itemstotal:
                        ...
                        TFC = threading.Thread(name='TFC'+str(i), target=FILMES_ANIMACAO_encontrar_fontes_filmes_TFC , args=(str(a),dads[0][0],dads[0][1],pesquisou,imdbcode,it,))
                        threads.append(TFC)
                        TFC.start()
                        TFC.join()
...
##                        TFC.join()
	...
	if nomesite != 'CMC':
                ...
                for item in items:
                        ...
                        a = str(i)
                        if i < 10: a = '0'+a
                        CMC = threading.Thread(name='CMC'+str(i), target=FILMES_ANIMACAO_encontrar_fontes_filmes_CMC, args=(str(a),url_pesquisa,pesquisou,imdbcode,itemi,))
                        threads.append(CMC)
                        CMC.start()
                        CMC.join()
...
##        [i.join() for i in threads]
        ...
        numstream = 0
        i = 0
        #if 'SITESdosPORTUGAS' not in resultados: resultados = []
##        for l in range(len(resultados)):
##                if resultados[l] != '': _nomeservidor_.append(resultados[l])
        if resultados != []:
...
def FILMES_ANIMACAO_encontrar_fontes_filmes_TFC(ordem,urle,titulo,pesquisou,imdbc,item):
        pt_en = 0
        num_f = 0
        if item != '':
                if item != '':
                                ...
                                try:
                                        if imdbc != '' and imdbcode != '':
                                                if imdbcode == imdbc:
                                                        ...
                                                        #addDir1(name,'url',1001,iconimage,False,fanart)
                                                        TFC_links('[B][COLOR green]' + nome + '[/COLOR][/B][COLOR yellow] (' + ano + ')[/COLOR]',urle+'IMDB'+imdbcode+'IMDB',iconimage,fanart)
                                                        num_f = num_f + 1
                                        else:
                                                try:
                                                        nnnn = re.compile('(.+?)[[].+?[]]').findall(nome)
                        ...
                        for servidor,matchsvids in matchvid:
                                ...
                                if '</iframe>' in matchsvids:
                                        ...
                                        if videomeg:
                                                ...
                                                PLAY_movie_url(urlvideomega,fonte_id,iconimage,'',fanart,'TUGAFILMES.TV')
...
def TFC_resolve_not_videomega_filmes(name,url,id_video,conta_id_video,conta_os_items,iconimage,fanart,urlvideomega):
...
def TPT_links(nomeescolha,urlescolha,iconimage,fanart):#,genre,plot,year):
        ...
        if not urlimdb:
                url = urlescolha.replace('IMDBIMDB','')
                urlvideomega = url
        else:
                url = urlimdb[0]
                urlvideomega = url
...
def TPT_resolve_not_videomega_filmes(url,conta_id_video,conta_os_items,nomeescolha,iconimage,fanart,urlvideomega):
        ...
        for fonte_e_url in fontes_video:
                                ...
                                try:  
                                        if 'hashkey' in fonte_id:
                                                ...
                                                if urlvideomega != '':
                                                        ...
                                                        PLAY_movie_url(urlvideomega,fonte_serv,iconimage,'',fanart,'CINEMATUGA.EU')
                                                        #addDir('[B]- Fonte ' + str(conta_id_video) + ' : [COLOR yellow](Videomega)[/COLOR][/B]',url,30,iconimage,'',fanart)
                ...
                for fonte_id in match2:
                        if 'videomega' in fonte_id:
                                ...
                                conta_id_video = conta_id_video + 1
                                CME_resolve_not_videomega_filmes(fonte_id,nomeescolha,conta_id_video,fanart,iconimage,urlvideomega)


def CME_resolve_not_videomega_filmes(url,nomeescolha,conta_id_video,fanart,iconimage,urlvideomega):
        url = url + '///' + nomeescolha
        if "videomega" in url:
		try:
                        url = urlvideomega
...
def CMC_resolve_not_videomega_filmes(url,conta_id_video,conta_os_items,nomeescolha,iconimage,fanart,urlvideomega):
...
def CMC_links(name,url,iconimage,fanart):
        ...
        for fonte_e_url in fontes_video:
                ...
                for match in matchs:
                        ...
                        url = match
                        conta_os_items = conta_os_items + 1
                        CMC_resolve_not_videomega_filmes(url,conta_id_video,conta_os_items,nomeescolha,iconimage,fanart,urlvideomega)
                                ...
                                for fonte_ids in fontes:
                                        if 'videomega' in fonte_ids:
                                                try:
                                                        ...
                                                        url = 'http://videomega.tv/iframe.php?ref=' + id_video# + '///' + name
                                                        fonte_serv = '(Videomega)'
                                                        PLAY_movie_url(urlvideomega,fonte_serv,iconimage,'',fanart,'MOVIETUGA')
                                                ...
                                                if match:
                                                        url = match[0]
                                                        conta_id_video = conta_id_video + 1
                                                        MVT_encontrar_streams(url,nomeescolha,iconimage,fanart,conta_id_video,urlvideomega)
                else:
 	                if fonte_video:
...
def MVT_encontrar_streams(url,nomeescolha,iconimage,fanart,conta_id_video,urlvideomega):
        if "videomega" in url:
		try:
                        url = urlvideomega
                        fonte_id = '(Videomega)'#+url
        ...
        for fonte_e_url in fontes_video:
                                ...
                                try:  
                                        if 'hashkey' in fonte_id:
                                                ...
                                                if urlvideomega != '':
                                                        ...
                                                        if urlvidlink: url = 'http://videomega.tv/iframe.php?ref=' + urlvidlink[0] + '///' + name
                                                        fonte_serv = '(Videomega)'
                                                        PLAY_movie_url(urlvideomega,fonte_serv,iconimage,'',fanart,'FOITATUGA')
                                                        #addDir('[B]- Fonte ' + str(conta_id_video) + ' : [COLOR yellow](Videomega)[/COLOR][/B]',url,30,iconimage,'',fanart)
                                        if 'iframe.js' in fonte_id:
                                                conta_id_video = conta_id_video + 1
                                                refvideo = re.compile('<script type="text/javascript">ref="(.+?)".+?</script>').findall(fonte_e_url)
                ...
                for fonte_id in match2:
                        if 'videomega' in fonte_id:
                                ...
                                conta_id_video = conta_id_video + 1
                                FTT_resolve_not_videomega_filmes(fonte_id,nomeescolha,conta_id_video,fanart,iconimage,urlvideomega)


def FTT_resolve_not_videomega_filmes(url,nomeescolha,conta_id_video,fanart,iconimage,urlvideomega):
        url = url + '///' + nomeescolha
        if "videomega" in url:
		try:
                        url = urlvideomega
...
##                        addLink(url,'','','')
	...
	if "putlocker" in url:
    	...
    	if "videomega" in url or 'Videomega' in name:
		try:
...
def videomega_resolver1(referer):
...
def videomega_resolver(referer):
	...
	if 'tuga-filmes.com' in referer:
                ...
                if re.search('http://videomega.tv/iframe.js',html):
                        ...
                        try:
                                hashk = re.compile('"http://videomega.tv/validatehash.php\?hashkey\=(.+?)"').findall(html)[0]
                                ref = re.compile('ref="(.+?)"').findall(requests.get("http://videomega.tv/validatehash.php?hashkey="+hashk,headers=ref_data).text)[0]
                        except:
                                try:
	...
	#addLink(url_video+headers_str(ref_data),'','','')
	#addLink(referer,'','','')
	return [url_video+headers_str(ref_data),url_legendas]
